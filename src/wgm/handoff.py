...
def validate_handoff(handoff: object) -> list[HandoffError]:
    ...
    for name in ("task_id", "capability"):
        if not isinstance(handoff.get(name), str) or not handoff[name]:
            errors.append(HandoffError(name, f"{name} must be a non-empty string"))
    ...
    if not isinstance(references, list) or not references or any(not isinstance(value, str) or not value for value in references):
        errors.append(HandoffError("evidence_references", "evidence_references must be a non-empty list of identifiers"))
    return errors
